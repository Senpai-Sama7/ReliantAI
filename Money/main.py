...
from database import init_db, save_dispatch, update_dispatch_status, get_dispatch, get_recent_dispatches, log_message
...
def _validate_login_csrf_token(form_token: str | None, cookie_token: str | None) -> bool:
    if not form_token or not cookie_token:
        return False
    if not hmac.compare_digest(form_token, cookie_token):
        return False
    try:
        data = _signer.loads(form_token, salt=_LOGIN_CSRF_SALT, max_age=_LOGIN_CSRF_MAX_AGE)
    except (BadSignature, SignatureExpired):
        return False
...
if os.path.exists(frontend_assets):
    ...
    logger.warning("⚠️ Frontend not built. Run: cd frontend && npm install && npm run build")


_cors_raw = os.environ.get("CORS_ORIGINS", "")
_cors_origins = [o.strip() for o in _cors_raw.split(",") if o.strip()] if _cors_raw else []
# In dev, allow localhost origins
if ENV == "dev" or not _cors_origins:
...
# In-memory job store (supplement to SQLite — keeps async status)
job_store: dict = {}
...
def _prune_stores():
    ...
    now = time.time()
    stale = [k for k, dq in _rate_counters.items()
    ...
    def call(self, func, *args, **kwargs):
        if self.state == "OPEN":
            if time.time() - self.last_failure_time > self.recovery_timeout:
                ...
                raise HTTPException(status_code=503, detail="Auth Service unavailable (Circuit Open)")
        ...
        try:
            ...
            if self.failures >= self.failure_threshold:
                self.state = "OPEN"
            logger.error("auth_service_unavailable auth_service_url=%s error=%s", AUTH_SERVICE_URL, exc)
...
def _authorize_request(x_api_key: str | None, request: Request | None = None) -> None:
    ...
    if not _api_key_configured():
        raise HTTPException(status_code=503, detail="DISPATCH_API_KEY is not configured")
...
def _publish_dispatch_completed_event(dispatch_id: str, result: dict) -> dict[str, str] | None:
    ...
    if EventPublishRequest and EventType:
...
def _twilio_skip_allowed() -> bool:
    ...
    if skip and ENV == "production":
        raise HTTPException(status_code=403, detail="Twilio validation may not be skipped in production")
    return skip
...
def _execute_job(run_id: str, message: str, temp: float):
    ...
    try:
        ...
        if os.environ.get("ENV") == "test":
            def run_hvac_crew(customer_message, outdoor_temp_f):
                from triage import triage_urgency_local
                res = triage_urgency_local(customer_message, outdoor_temp_f)
                return {
                    "raw_output": f"Mock engine dispatched: {res['urgency_level']}",
                    "confidence": 0.99,
                    "metadata": res
                }
        else:
            try:
                ...
                logger.warning("CrewAI missing, using pure triage fallback mode: %s", e)
                def run_hvac_crew(customer_message, outdoor_temp_f):
                    from triage import triage_urgency_local
                    res = triage_urgency_local(customer_message, outdoor_temp_f)
                    return {
        ...
        sm.record_event(run_id, 'dispatch_completed', actor='hvac_crew', run_id=run_id, payload=result)
        ...
        if DISPATCH_JOB_DURATION:
            DISPATCH_JOB_DURATION.labels(status="success").observe(time.time() - start_time)
            
    except Exception as exc:
        logger.error("Job %s failed: %s", run_id, exc)
        if DISPATCH_JOB_DURATION:
            DISPATCH_JOB_DURATION.labels(status="error").observe(time.time() - start_time)
        sm.record_event(run_id, 'error', actor='system', run_id=run_id, payload={'error': str(exc)})
...
@app.get("/metrics")
def metrics(x_api_key: str = Header(default=None)):
    """Expose Prometheus metrics."""
    return get_metrics_response()


@app.post("/dispatch", response_model=DispatchResponse, responses={401: {"model": ErrorResponse}})
def dispatch(
    ...
    """API-driven dispatch (for internal/dashboard use)."""
...
@app.get("/dispatches")
def list_dispatches(request: Request, limit: int = 50, x_api_key: str = Header(default=None)):
...
def get_dispatch_funnel(
    ...
    active = total - counts.get('cancelled', 0) - counts.get('escalated', 0) - counts.get('followed_up', 0)
...
async def sms_webhook(
    ...
    # Anti-spam rate limiting: drop texts from the same number faster than 5 seconds
    now_time = time.time()
    last_hit = rate_limit_store.get(From, 0.0)
    if now_time - last_hit < 5.0:
        logger.warning("Rate limit hit from %s, dropping SMS", From)
        return _twiml_response("") # Empty response does not echo
    rate_limit_store[From] = now_time

    # Truncate inbound body (Anti-RAM-DoS)
    Body = Body[:MAX_MSG_LENGTH]
    
    # Strip HTML tags to prevent XSS echoing
    import re
    Body = re.sub(r'<[^>]+>', '[REDACTED]', Body)
    
    logger.info("SMS received from %s: %s", From, Body[:100])
    log_message(direction="inbound", phone=From, body=Body, sms_sid=MessageSid, channel="sms")

    # Fire dispatch in background; respond immediately with acknowledgment
    run_id = str(uuid.uuid4())
    _prune_stores()
    job_store[run_id] = {"status": "queued", "result": None}
    ...
    sm = get_state_machine()
    sm.transition(run_id, DispatchState.RECEIVED, actor='customer', payload={'channel': 'sms', 'phone': From})

    background_tasks.add_task(_execute_job, run_id, Body, 95.0)  # Default Houston summer temp

    # Generate 4-beat structured response (7-beat model adapted for SMS)
    from response_templates import generate_dispatch_response
    from triage import triage_urgency_local
    
    # Quick triage to determine urgency for appropriate response
...
async def whatsapp_webhook(
    ...
    # Anti-spam rate limiting (WhatsApp)
    now_time = time.time()
    last_hit = rate_limit_store.get(From, 0.0)
    if now_time - last_hit < 5.0:
        logger.warning("Rate limit hit from %s, dropping WhatsApp", From)
    ...
    logger.info("WhatsApp received from %s: %s", From, Body[:100])
    log_message(direction="inbound", phone=From, body=Body, sms_sid=MessageSid, channel="whatsapp")
    ...
    from state_machine import get_state_machine, DispatchState
    sm = get_state_machine()
    sm.transition(run_id, DispatchState.RECEIVED, actor='customer', payload={'channel': 'whatsapp', 'phone': From})

    background_tasks.add_task(_execute_job, run_id, Body, 95.0)

    # Generate 4-beat structured response (7-beat model adapted for WhatsApp)
    from response_templates import generate_dispatch_response
    from triage import triage_urgency_local
...
def login_submit(
    ...
    if not _validate_login_csrf_token(csrf_token, cookie_token):
        return _render_login_page(request, "Invalid or expired CSRF token", status_code=403)
...
def admin_dashboard(request: Request):
    ...
    total = len(recent)
    emergency_count = sum(1 for d in recent if d.get("urgency", "").lower() == "emergency")
    completed_count = sum(1 for d in recent if d.get("status", "").lower() == "complete")
