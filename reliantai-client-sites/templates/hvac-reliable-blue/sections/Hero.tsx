...
export default function Hero({ content }: HeroProps) {
  ...
  return (
    <section className="relative min-h-screen flex items-center pt-12 overflow-hidden bg-gradient-to-b from-slate-950 via-blue-950/60 to-slate-950">
      {/* Layered gradient blobs — replaces the old grid overlay */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-40 -right-40 w-[640px] h-[640px] rounded-full bg-blue-600/5 blur-3xl" />
        <div className="absolute top-1/3 -left-20 w-[400px] h-[400px] rounded-full bg-blue-500/4 blur-3xl" />
        <div className="absolute bottom-0 right-1/4 w-[300px] h-[300px] rounded-full bg-cyan-600/4 blur-3xl" />
      </div>

      {/* Radial glow behind the visual element */}
      <div
        className="absolute top-1/2 right-0 -translate-y-1/2 w-[800px] h-[600px] pointer-events-none"
        style={{
          background: "radial-gradient(ellipse at center, rgba(96,165,250,0.12) 0%, rgba(96,165,250,0.03) 50%, transparent 75%)",
        }}
      />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          >
            <div className="relative w-full max-w-md aspect-square">
              {/* Outer glow ring */}
              <div className="absolute inset-0 rounded-full bg-gradient-to-br from-blue-500/15 via-blue-600/5 to-transparent blur-2xl" />
              {/* Center orb */}
              <div className="absolute inset-[15%] rounded-full bg-gradient-to-br from-blue-600/20 via-blue-500/10 to-blue-900/5 border border-blue-500/10 shadow-[0_0_120px_rgba(59,130,246,0.08)]" />
              {/* Inner accent */}
              <div className="absolute inset-[35%] rounded-full bg-gradient-to-tr from-blue-400/10 to-transparent" />
              {/* Floating smaller orbs */}
              <div className="absolute top-[5%] right-[10%] w-20 h-20 rounded-full bg-cyan-500/10 blur-xl" />
              <div className="absolute bottom-[10%] left-[5%] w-16 h-16 rounded-full bg-blue-400/8 blur-xl" />
            </div>
          </motion.div>
        </div>
      </div>
