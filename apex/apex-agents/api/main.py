...
try:
    ...
    print(f"APEX Auth: {'ENABLED' if AUTH_ENABLED else 'DISABLED'}")
except ImportError as e:
    print(f"WARNING: Auth integration not available: {e}")
    AUTH_ENABLED = False
    auth = None
...
# Layer 1 import — defensive because module name may vary across build states
try:
    from agents.layer1.workflow import run_layer1
except ImportError:
    try:
        from agents.layer1.workflow import run_layer1_workflow as run_layer1  # type: ignore
    except ImportError:
        run_layer1 = None  # type: ignore
...
app.add_middleware(
    ...
    allow_origins=[
        origin.strip()
        for origin in os.getenv("APEX_CORS_ORIGINS", "http://localhost:3000").split(",")
        if origin.strip()
    ],
...
async def health():
    return {
        ...
        "service":          "apex-agents",
        "version":          "0.1.0",
        "layer1_available": run_layer1 is not None,
    }
...
async def layer1_analyze(
    ...
    if run_layer1 is None:
        raise HTTPException(503, "Layer 1 not loaded — check agents/layer1/workflow.py")
...
async def submit_hitl_decision(
    ...
    try:
        ...
        await conn.execute(
            ...
            WHERE id = $4::uuid
            """,
            status, body.rationale, body.reviewer, decision_id,
        )
        await conn.execute(
            """
            ...
            """,
            status, body.reviewer, body.rationale, decision_id,
        )
...
async def _create_hitl_record(
    ...
    def _to_json(obj: Any) -> str:
        ...
        if hasattr(obj, "model_dump"):     return json.dumps(obj.model_dump())
    ...
    row = await conn.fetchrow(
        ...
        RETURNING id::text
        """,
        trace_id, task[:500], routing_tier, _to_json(l3_output), _to_json(l4_review),
    )
    return row["id"]
...
async def run_workflow(
    ...
    # ── Layer 1 ──────────────────────────────────────────────────────────────
    if run_layer1 is None:
        raise HTTPException(503, "Layer 1 not available.")

    l1_state = await run_layer1(
        task=req.task, context=req.context, trace_id=trace_id
    )
    ...
    requires_hitl  = bool(_extract(evolver_out, "requires_hitl",         True)  if evolver_out else True)
    requires_rerun = bool(_extract(evolver_out, "requires_layer3_rerun",  False) if evolver_out else False)

    # One L3 rerun if Evolver flags it
    if not final_approved and requires_rerun and req.allow_rerun:
        revision_notes = str(_extract(evolver_out, "convergence_note", "") if evolver_out else "")
...
async def auth_status():
    """Get authentication status for APEX."""
    return {
        "auth_enabled": AUTH_ENABLED,
        "service": "apex-agents",
        "shared_library_path": "/home/donovan/Projects/ReliantAI/integration/shared"
    }
