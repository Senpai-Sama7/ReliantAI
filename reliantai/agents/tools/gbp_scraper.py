...
class GBPScraperTool(BaseTool):
                ...
                "reviews": [
                    {
                        ...
                        "has_response": bool(r.get("author_name") and "owner" in r.get("author_name", "").lower()),
                    }
                    for r in reviews[:5]
                ],
            }
    ...
    def _estimate_review_response_rate(self, reviews: list) -> float:
        if not reviews:
            return 0.0
        with_response = sum(1 for r in reviews if r.get("author_name", "").lower().startswith("owner"))
