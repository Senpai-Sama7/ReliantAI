...
async def run_workflow(
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        try:
            response = await client.post(
                f"{APEX_AGENTS_URL}/workflow/run",
                json=req.input_data
            )
...
async def layer1_analyze(
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{APEX_AGENTS_URL}/agents/layer1/analyze",
            json=data
        )
        return response.json()
...
async def layer2_calibration(
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{APEX_AGENTS_URL}/agents/layer2/calibration-gate",
            json=data
        )
        return response.json()
...
async def layer3_dispatch(
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{APEX_AGENTS_URL}/agents/layer3/dispatch",
            json=data
        )
        return response.json()
