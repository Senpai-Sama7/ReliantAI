...
CITADEL_ORCHESTRATOR_URL = os.getenv(
    "CITADEL_ORCHESTRATOR_URL", "http://localhost:8000"
)
...
# Endpoints
@app.post("/query")
async def query(req: QueryRequest, user: Dict[str, Any] = Depends(verify_token)):
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        try:
            response = await client.post(
                f"{CITADEL_ORCHESTRATOR_URL}/query", json={"query": req.query}
            )
            ...
            raise HTTPException(
                status_code=503, detail="Citadel orchestrator unavailable"
            )
...
async def vector_search(
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{CITADEL_ORCHESTRATOR_URL}/vector/search", json=data
        )
        return response.json()


@app.post("/knowledge-graph/query")
async def kg_query(data: Dict[str, Any], user: Dict[str, Any] = Depends(verify_token)):
    """Knowledge graph query with auth"""
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{CITADEL_ORCHESTRATOR_URL}/knowledge-graph/query", json=data
        )
        return response.json()
