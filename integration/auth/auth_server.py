...
token_validation = Counter("auth_token_validation_total", "Total token validations", ["status"])
login_failures = Counter("auth_login_failures_total", "Total login failures", ["reason"])
request_duration = Histogram("auth_request_duration_seconds", "Request duration", ["endpoint"])
rate_limit_rejections = Counter("auth_rate_limit_rejections_total", "Rejected auth requests", ["endpoint"])
...
async def get_user(username: str) -> Optional[User]:
    ...
    if redis_client is not None:
        try:
            ...
            logger.warning("redis_user_lookup_failed", username=username, error=str(exc))
        else:
            if user_data:
...
async def store_user(user: User):
    ...
    try:
        ...
        logger.warning("redis_user_cache_write_failed", username=user.username, error=str(exc))
...
async def login(request: Request, background_tasks: BackgroundTasks, form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    with request_duration.labels(endpoint="token").time():
        ...
        access_token = create_token(
            {"sub": user.username, "tenant_id": user.tenant_id, "role": user.role.value, "type": "access"},
...
async def refresh(refresh_token: str):
    ...
    with request_duration.labels(endpoint="refresh").time():
        try:
            ...
            access_token = create_token(
                {"sub": username, "tenant_id": tenant_id, "role": user.role.value, "type": "access"},
                timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
            )
...
async def health():
    ...
    try:
        await redis_client.ping()
        sqlite_status = {"journal_mode": "unknown", "user_count": "0"}
        if user_store is not None:
            sqlite_status = await user_store.ping()
        return {
            "status": "healthy",
            "redis": "connected",
            "sqlite": {"status": "connected", **sqlite_status, "path": str(get_auth_db_path())},
        }
    except Exception as e:
        raise HTTPException(status_code=503, detail=f"Dependency unavailable: {str(e)}")
...
async def protected_read(current_user: dict = Depends(require_permission(Permission.READ))):
...
async def protected_write(current_user: dict = Depends(require_permission(Permission.WRITE))):
...
async def protected_delete(current_user: dict = Depends(require_permission(Permission.DELETE))):
    """Test endpoint requiring DELETE permission"""
    return {"message": "Delete access granted", "user": current_user["username"]}

@app.get("/protected/admin")
async def protected_admin(current_user: dict = Depends(require_permission(Permission.ADMIN))):
